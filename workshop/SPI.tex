...
\label{sec:PI_causes}
In this section we describe potential violations of SPI that can occur in a 
distributed setting. We also explain how we avoid these issues by using special 
...
sequences of \emph{scans}---a single call to \cref{alg:distributed_PT}---of 
exponentially increasing length. At the end of each round,
\emph{replicas}---the fundamental unit of parallelization in PT (see \cref{sec:PT} for 
details)---need to exchange information. This can be related to the output of the 
