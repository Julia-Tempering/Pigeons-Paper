...
\begin{figure*}[t]
    ...
    \end{minipage}
    \caption{
        Weak scaling results for PT in both the distributed (MPI) and parallel settings 
        with approximate 95\% confidence intervals for the speedup.  
        ...
        \textbf{Left:} Distributed setup. 
        \textbf{Right:} Parallel (shared memory) setup. 
    }
...
For instance, in some situations the likelihood evaluation of a single chain 
requires its own GPU (e.g., for high-resolution black hole imaging, which relies 
on running a Fast Fourier Transform on a GPU). 
